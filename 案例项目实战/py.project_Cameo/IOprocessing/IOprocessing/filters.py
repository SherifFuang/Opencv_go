...
def strokeEdges(src,dst,blurKsize = 7,edgeKsize = 5):
    if blurKsize >= 3:
        blurredSrc = cv2.medianBlur(src,blurKsize)#对彩色图像用了medianblur的模糊函数
        graySrc = cv2.cvtColor(blurredSrc, cv2.COLOR_BAYER_BG2BGRA)#在使用拉普拉斯之前要彩色空间变换变成灰度图，提高边缘检测效果
    else:
        graySrc = cv2.cvtColor(src,cv2.COLOR_BGR2GRAY)
    cv2.Laplacian(graySrc,cv2.CV_8U,graySrc,ksize = edgeKsize)#用拉普拉斯来提取边缘，产生明显的边缘线条
    normalizedInverseAlpha = (1.0 / 255) * (255 - graySrc)#归一化图像，
    channels = cv2.split(src)
    for channel in channels:
        channel[:] = channel * normalizedInverseAlpha#归一化后的图像乘以原图像能使得边缘变黑
...
class VConvolutionFilter(object):#用一般的卷积滤波器filter2D
    """A filter applies a convolution to V(or all of BGR)"""
    def __init__(self,kernel):
        self._kernel = kernel
    def apply(self,arc,dst):
        """Apply the filter with a BGR or gray source /destination"""
    cv2.filter2D(src,-1,self.kernel,dst)

class SharpenFilter(VConvolutionFilter):#用自制的锐化卷积滤波器
...
class FindEdgeFilter(VConvolutionFilter):#自制边缘检测滤波器
    """A edge-finding filter with 1-pixel radius"""
    def __init__(self):
        kernel = numpy.array([-1,-1,-1],
                             [-1, 8,-1],
                             [-1,-1,-1])
...
class BlurFilter(VConvolutionFilter):#自制边缘模糊滤波器
...
class EmbossFilter(VConvolutionFilter):#一种有模糊和锐化作用的核，产生脊状和浮雕效果
